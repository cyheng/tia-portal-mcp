...
$buildOutput = Join-Path $RepositoryRoot "src/TiaMcpServer/bin/Release/net48"
$packageName = "TiaMcpServer-v21-win-x64"
$stageDirectory = Join-Path $OutputDirectory $packageName
$zipPath = Join-Path $OutputDirectory "$packageName.zip"
if (-not (Test-Path (Join-Path $buildOutput "TiaMcpServer.exe"))) {
    throw "请先在安装了 TIA Portal V21 Openness 的 Runner 上构建项目，生成 $buildOutput/TiaMcpServer.exe。"
}
